...
## Статус: выполнено, три кадра из пяти

Сгенерировано через ChatGPT 29.07.2026. Пригодными вышли три кадра, они приняты и
лежат в `images/`:

| Промпт | Файл | Секция |
|--------|------|--------|
| 1. Hero (колея) | `01-road.jpg` | 1 |
| 3. Мотоциклы | `02-moto.jpg` | 2 |
| 5. Финал (закат) | `03-sunset.jpg` | 3 |

Промпты 2 (дорога с деревьями на горизонте) и 4 (экипировка на досках) пригодного
результата не дали. Структура страницы сжата с пяти секций до трёх - см. основной
спек, раздел "Три секции".

Промпты ниже сохранены: пригодятся, если понадобится перегенерировать в большем
разрешении (текущие 941px меньше физической ширины экрана iPhone) или добавить
кадров.

Дальше - исходные условия, по которым промпты писались.
