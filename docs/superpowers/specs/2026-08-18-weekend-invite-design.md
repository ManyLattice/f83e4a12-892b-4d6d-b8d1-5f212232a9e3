...
1. **Сюжет с запасом по краям.** На широком экране срезается верх и
   низ, на узком - бока. Пара, плед и лошади должны пережить оба
   кадрирования.
2. **Верх спокойный.** Там лежит заголовок - нужна ровная область без
   мелких деталей.

Поверх иллюстраций лежит тёмный оверлей: он держит читаемость белого
заголовка независимо от того, что на картинке.
