...
- Наведение на сторону осветляет её и приглушает противоположную -
  дорога, к которой тянешься, становится ярче
- Тач: сторона на мгновение проседает под пальцем
- Вся сцена медленно «дышит» - едва заметный зум, чтобы экран не
  выглядел мёртвой картинкой

Пара в центре - нейтральная зона: тап по ней ничего не выбирает. Она
принадлежит обоим путям сразу, и заставлять её что-то значить было бы
против смысла кадра.
...
### Требования к кадру и почему развилка вписывается

Развилка показывается целиком (`contain`), а не заполняет экран. Причина
измеримая: плед лежит на 5.1% ширины кадра, лошади тянутся до 99.9% -
вместе 94.8%. Телефон при заполнении экрана показывает только 82%
ширины. Разрыв в 12 процентных пунктов не закрывается никакой точкой
фокуса: сдвинешь кадр к пледу - уйдут лошади, и наоборот.

Две перегенерации это не исправили - генератор каждый раз ставил
объекты у самых краёв, несмотря на прямое требование отступов. Поэтому
решение принято на стороне вёрстки.

Полосы сверху и снизу заполнены градиентом, продолжающим небо и землю
самой иллюстрации (`#AA946E` вверху, `#774C1E` внизу), поэтому страница
читается как одна картина, а не как фото в рамке. На iPhone 13 полосы
занимают около 9% высоты каждая.

Зоны выбора обрезаны по картинке: тап по полосе не выбирает дорогу.
Каллиграфия привязана к верхнему краю иллюстрации, а не экрана.

Две сцены после выбора заполняют экран (`cover`) - у них нет деталей по
краям, которые нельзя потерять.
